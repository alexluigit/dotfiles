...
local art = wibox.widget {
    ...
    forced_width = dpi(80),
    clip_shape = shapes.rrect(beautiful.border_radius - 5),
...
local create_button = function(symbol, color, command, playpause)
    ...
    local icon = wibox.widget {
        markup = colors.colorize_text(symbol, color),
    ...
    awesome.connect_signal("bling::playerctl::status", function(playing)
        if playpause then
            if playing then
                icon.markup = colors.colorize_text("", color)
            else
                icon.markup = colors.colorize_text("", color)
            end
        end
    ...
    button:connect_signal("mouse::enter", function()
        icon.markup = colors.colorize_text(icon.text, beautiful.xforeground)
    end)

    button:connect_signal("mouse::leave", function()
        icon.markup = colors.colorize_text(icon.text, color)
    end)
...
local slider = wibox.widget {
    forced_height = dpi(5),
    bar_shape = shapes.rrect(beautiful.border_radius),
