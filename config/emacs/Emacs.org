...
This is my Emacs configuration file written as an Org Mode document.  This file generates [[file:init.el][init.el]] which can be loaded by Emacs at startup.
...
The following variables are used to tweak some of the configuration pieces.
...
** Settings
*** Disable backup ('*.txt~') files.
#+begin_src emacs-lisp
(setq make-backup-files nil)
#+end_src
*** Disable message/scratch/completion buffer
#+begin_src emacs-lisp
;; Makes *scratch* empty.
...
#+begin_src emacs-lisp
  ...
  (use-package general
    ...
    "Open the init file."
      (interactive)
      (find-file "~/Dev/Alex.files/config/emacs/Emacs.org"))

    (alex/leader-keys
      "f"  '(:ignore t :which-key "find files")
      ;; "f." '((find-file "~/Dev/Alex.files/config/emacs/Emacs.org") :which-key "emacs config")
...
#+begin_src emacs-lisp
  ...
  (use-package evil
    ...
    (define-key evil-normal-state-map (kbd "C-p") 'evil-search-previous)
...
The =alex/org-font-setup= function configures various text faces to tweak the sizes of headings and use variable width fonts in most cases so that it looks more like we're editing a document in =org-mode=.  We switch back to fixed width (monospace) fonts for code blocks and tables so that they display correctly.
...
This snippet adds a hook to =org-mode= buffers so that =alex/org-babel-tangle-config= gets executed each time such a buffer gets saved.  This function checks to see if the file being saved is the Emacs.org file you're looking at right now, and if so, automatically exports the configuration here to the associated output files.

#+begin_src emacs-lisp

  ;; Automatically tangle our Emacs.org config file when we save it
  (defun alex/org-babel-tangle-config ()
    (when (string-equal (buffer-file-name)
                        (expand-file-name "~/Dev/Alex.files/config/emacs/Emacs.org"))
      ;; Dynamic scoping to the rescue
      (let ((org-confirm-babel-evaluate nil))
        (org-babel-tangle))))

  (add-hook 'org-mode-hook (lambda () (add-hook 'after-save-hook #'alex/org-babel-tangle-config)))
...
#+begin_src emacs-lisp
  ...
  (use-package typescript-mode
    ...
    '("node /home/alex/.local/share/lsp/eslint/server/out/eslintServer.js --stdio"))
